<div class="table-responsive">
    <table class="table">
        ...
        <tbody>
            @if (isset($data['terms_condition']) && count($data['terms_condition'])>0)
            @php $any_permission_found = false; @endphp
            @foreach ($data['terms_condition'] as $key => $item)
                @php
                   $sr_no = $key + 1;
                        if ($data['terms_condition']->currentPage()>1) {
                            $sr_no = ($data['terms_condition']->currentPage()-1)*$data['terms_condition']->perPage();
                            $sr_no = $sr_no + $key + 1;
                        }
                ...
                <tr data-user-id="{{ $item->id }}">
                    ...
                    <td>
                        @canany(['terms-condition-edit', 'terms-condition-delete', 'terms-condition-status', 'terms-condition-detail'])
                            <div class="dropdown">
                                <button type="button"
                                ...
                                <div class="dropdown-menu">
                                    @can('terms-condition-detail')
                                        <button type="button" class="dropdown-item" data-toggle="modal"
                                            data-target="#terms_condition-{{ $item->id }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    @can('terms-condition-edit')
                                        <a class="dropdown-item" href="{{ url('terms_condition') }}/{{ $item->id }}/edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    @can('terms-condition-delete')
                                        <form action="{{ url('terms_condition/' . $item['id']) }}" method="post">
                                            @method('delete')
                                            @csrf
                ...
                <div class="modal fade" id="terms_condition-{{ $item->id }}" tabindex="-1"
...
@endif
    ...
    <div class="pagination_links">
        @if (isset($data['terms_condition']) && count($data['terms_condition']) > 0)
            {{ $data['terms_condition']->links('vendor.pagination.bootstrap-4') }}
        @else
